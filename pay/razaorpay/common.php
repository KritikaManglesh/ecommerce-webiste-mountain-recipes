...
class STUDENT {
	...
	public function razorPayOnline($name,$email,$phone, $service, $typeProduct, $toValue, $message,$razorpayOrderId,$razorpayPaymentId,$paymentStatus,$makerstamp,$updatestamp) {
		try {
			$stmt = $this->conn->prepare( "INSERT INTO onlinepayment(name, email, phone, address,service, typeProduct, toValue, message, razorpayOrderId, razorpayPaymentId, paymentStatus,makerstamp,updatestamp) 	
			VALUES(:name_o,:email_o,:phone_o,:address_o,:service_o,:typeProduct_o,:toValue_o,:message_o,:razorpayOrderId_o,:razorpayPaymentId_o,:paymentStatus_o,:makerstamp_o,:updatestamp_o)" );
